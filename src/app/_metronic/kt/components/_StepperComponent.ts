...
class StepperComponent {
  ...
  constructor(_element: HTMLElement, options: IStepperOptions) {
    ...
    // Variables
    this.totalStepNumber = this.steps?.length | 0;
  ...
  private _goTo = (index: number) => {
    ...
    if (index === this.currentStepIndex || index > this.totalStepNumber || index < 0) {
      return;
    }
  ...
  private isLastStep = () => {
    return this.currentStepIndex === this.totalStepNumber;
  };
  ...
  public getNextStepIndex = () => {
    if (this.totalStepNumber >= this.currentStepIndex + 1) {
      return this.currentStepIndex + 1;
    } else {
      return this.totalStepNumber;
    }
