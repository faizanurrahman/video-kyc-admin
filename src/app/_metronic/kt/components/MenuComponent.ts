...
/**
 * The MenuComponent class represents a menu component with dropdown and accordion functionality.
 */
class MenuComponent {
  // An instance of this class always has an element property that is an HTMLElement.
  element: HTMLElement;

  // An instance of this class can also have an options property that is an object.
  options: MenuOptions;

  // An instance of this class has a unique ID that is stored in the instanceUid property.
  instanceUid: string;

  // An instance of this class can have a triggerElement property that is either null or an HTMLElement.
  triggerElement: HTMLElement | null = null;

  // This is a constructor function that initializes a Menu instance.
  // It takes two arguments:
  //    - _element: an HTMLElement that represents the root element of the menu.
  //    - options: an object that contains additional options to customize the menu.

  constructor(_element: HTMLElement, options: MenuOptions) {
    // Testing Purpose, In loan calculator checking menu

    // Store the element and options properties on this object.
    ...
    // Commented out line. Possibly was used to mark the element as a Kt menu.
    // this.element.setAttribute('data-kt-menu', 'true')

    // Set up event listeners for the trigger element(s).
    this._setTriggerElement();

    // Initial update of the menu position and dimensions.
    this._update();

    // Store a reference to this object on the element's dataset.
  ...
  /**
   * This private method sets the trigger element for a menu based on the given element.
   * If it finds a suitable trigger element, it associates the menu instance with the trigger element
   * using the DataUtil class (which is assumed to store data in a hashmap).
   */
  private _setTriggerElement = () => {
    // Try to find an element with a data-kt-menu-target attribute that matches the current element's id.
    const target = document.querySelector(
      `[data-kt-menu-target="#${this.element.getAttribute('id')}"`,
    );

    if (target) {
      // console.log('target menu found at ', target);
      // If found, assign it to the triggerElement property.
      this.triggerElement = target as HTMLElement;
    } else if (this.element.closest('[data-kt-menu-trigger]')) {
      // console.log(
      //   'finding closest menu trigger item',
      //   this.element.closest('[data-kt-menu-trigger'),
      // );
      // If not found, try to find the closest ancestor element that has a data-kt-menu-trigger attribute.
      // If found, assign it to the triggerElement property.
      ...
      // If still not found, try to find the first child element of the current element's parent that has a data-kt-menu-trigger attribute.
      // If found, assign it to the triggerElement property.
    ...
    // If a trigger element was found, associate the menu instance with it using the DataUtil class.
      ...
      if (menuItems && menuItems.length > 0) {
        for (let i = 0; i < menuItems.length; i++) {
          ...
          if (menuObj && menuObj.getItemSubType(item) === 'dropdown') {
            ...
            // if menu has attributes data-kt-permanent
            if (
              item.hasAttribute('data-kt-menu-permanent') &&
              item.getAttribute('data-kt-menu-permanent') === 'true'
            ) {
              continue;
            }
