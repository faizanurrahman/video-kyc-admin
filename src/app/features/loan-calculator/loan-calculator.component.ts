...
export class LoanCalculatorComponent implements AfterViewInit {
  ...
  private async getLoanCalculatedData() {
    ...
    const payload = {
      ...
      sessionId: sessionId,
      data: {
        LOAN_TERMS: this.N.toFixed(0).toString(), // fix: now N is in months
        ...
        .subscribe((res: any) => {
          if (res.status === 'FAILED') {
            ...
            Swal.fire({
              ...
              title: 'Oops...',
              heightAuto: false,
              text: 'Dear Customer , Please enter the valid interest rate between 2% to 9%.',
            });
            this.showCalculateButton = true;
