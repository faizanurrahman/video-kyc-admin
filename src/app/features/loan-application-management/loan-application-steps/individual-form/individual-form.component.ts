...
import { IndividualCountService } from './individual-count.service';
import {
  IndividualDetailsModel,
  IndividualsCreateBody,
  IndividualsCreateResponse,
} from './individual-form.model';
import { differentKinAddressesValidator } from './kin-address-validator';
import { kinContactsWithinDifferentValidator } from './kin-contact-within-individual-unique-validator';
import { MobileNumberVerificationInputComponent } from './mobile-number-verification-input/mobile-number-verification-input.component';
import { omangNumberUniqueValidator } from './omang-number-unique-validator';
import { primaryMobileNumberUniqueValidator } from './primary-contact-across-individual.validator';
import { RequiredIndividualFields } from './required-field';
...
export class IndividualFormComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    console.log('On init calling form patch individuals');
  ...
  private patchIndividualDetails() {
    ...
    console.log(
      'Path Individual Details is calling add Individuals with: ',
  ...
  private individualFormInit() {
    ...
    if (
      ...
      console.log(
        'whilte form initialization, we are calling add individual with default 5 individual',
      );
      // this.addIndividuals(5);
    }

    this.individualForm.valueChanges.subscribe((res: any) => {
      console.log('Value Changes in Individual On Load: ', res, '%cfont-size: 30px');
    });

    this.individualForm.statusChanges.subscribe((status: any) => {
      console.log('Status Changes on Load', status, '%ccolor: red; font-size: 30px');
    });
  ...
  public async addIndividuals(numberOfIndividuals: number) {
    console.log('Add Individual Called with ', numberOfIndividuals, ' number of individuals');
    // if (
  ...
  public async saveApplication(): Promise<boolean> {
    ...
    try {
      ...
      console.log('save application calling form patch individuals');
      this.patchIndividualDetails();
